...
export async function captureNormalizedScreenshot(
  ...
  let clipOpts: Parameters<Page["screenshot"]>[0] | undefined;
  if (fullPage) {
  ...
  if (opts?.selector) {
    const buffer = await page.locator(opts.selector).screenshot({ type: "png" }) as Buffer;
  ...
  // Attempt PNG capture
  const pngOpts = clipOpts ?? { type: "png" as const, fullPage };
  const pngBuffer = await page.screenshot(pngOpts) as Buffer;
...
async function reduceToJpeg(
  page: Page,
  _fallbackBuffer: Buffer,
  maxBytes: number,
  selector?: string,
