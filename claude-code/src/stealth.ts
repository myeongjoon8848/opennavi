...
const OPENNAVI_DIR = join(homedir(), ".opennavi");

export function resolveUserDataDir(profileName = "default"): string {
  return join(OPENNAVI_DIR, "browser", profileName, "user-data");
}
...
export function buildStealthLaunchArgs(): string[] {
  return [
    // --- Anti-detection ---
...
export function pickUserAgent(): string {
  if (process.platform === "darwin") return DESKTOP_USER_AGENTS[0]!;
  if (process.platform === "win32") return DESKTOP_USER_AGENTS[1]!;
