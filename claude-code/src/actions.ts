...
export async function executeAct(page: Page, request: ActRequest): Promise<unknown> {
  switch (request.kind) {
    case "click": {
      const ref = requireRef(request);
      const locator = refLocator(page, ref);
      const opts: Parameters<typeof locator.click>[0] = {};
    ...
    case "evaluate": {
      const fn = request.fn;
      if (!fn) throw new Error("fn is required for evaluate");
    ...
    case "batch": {
      ...
      if (actions.length > 100) throw new Error("batch supports at most 100 actions");
      ...
      for (let i = 0; i < actions.length; i++) {
        ...
        if (action.kind === "batch") throw new Error("nested batch is not allowed");
        try {
