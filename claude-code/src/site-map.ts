...
interface SiteRecord {
  domain?: string;
  nodes?: Record<string, { url?: string }>;
}
...
function parsePattern(raw: string): { path: string; query: Map<string, string> } | null {
...
function matchesPath(currentPath: string, patternPath: string): boolean {
  ...
  for (let i = 0; i < pat.length; i++) {
    if (isVariable(pat[i])) continue;
    if (pat[i] !== cur[i]) return false;
  }
  return true;
}
...
export function matchesNodePattern(currentUrl: string, nodePattern: string): boolean {
...
export function findMatchingNode(url: string, record: SiteRecord): string | null {
