...
async function stopChrome() {
    ...
    if (process.platform === "darwin") {
        ...
        try {
            (0, node_child_process_1.execFileSync)("pkill", ["-f", "chrome|chromium|brave|msedge"], { timeout: 3000 });
        }
        catch { }
    }
...
async function connectBrowserInternal() {
    ...
    if (!browser?.isConnected()) {
        ...
        if (errMsg.includes("setDownloadBehavior") || errMsg.includes("context management is not supported")) {
            throw new errors_js_1.BrowserConnectionError([
    ...
    if (contexts.length === 0) {
        throw new errors_js_1.BrowserConnectionError("Chrome에 연결했지만 활성 브라우저 컨텍스트가 없습니다.\nChrome 창을 하나 열고 다시 시도해주세요.");
    }
    context = contexts[0];
...
function getPage(targetId) {
    ...
    if (entries.length > 1) {
        ...
        throw new errors_js_1.BrowserTabNotFoundError(`Multiple tabs open (${tabList}) — targetId is required. Use action="tabs" to list tabs, then pass targetId to target a specific tab.`);
    }
